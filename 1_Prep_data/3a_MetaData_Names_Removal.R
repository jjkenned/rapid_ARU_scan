...
for (i in 1:nrow(meta_2)){
  ...
  meta_2$ynight[i] = or.night(meta_2$timestamp[i],12,"Etc/GMT+8")
...
meta_2 = meta_3 %>% rename(True_Lat = Ext_Lat,True_Lon = Ext_Lon,
                           Int_Loc = location,start_time = timestamp)
...
get_end_time = function(start,format,timezone,length){
  ...
  end = as.POSIXct(start, tz = as.character(timezone))+ceiling(seconds(length))
...
for (i in 1:nrow(meta_2)){
  ...
  meta_2$end_time[i] = get_end_time(meta_2$start_time[i],time_format,timezone,meta_2$length[i])
...
for (station in unique(meta_2$station)){
  ...
  for (ynight in unique(meta_stn$ynight)){
    ...
    date_range = unique(date(as.POSIXct(meta_night$start_time, tz = timezone)))
    ...
    sunset = suncalc::getSunlightTimes(date = date(min(date_range)),lat = meta_night$Int_Lat[1],lon = meta_night$Int_Lon[1], keep = c("sunset"))
    ...
    for (i in 1:nrow(meta_night)){
      ...
      meta_rec = meta_night[i,c("filename","station","ynight","start_time","end_time","length","status","sunrisetime","sunsettime","true_sunrise","true_sunset","Int_Lon","Int_Lat","True_Lat","True_Lon")]
      ...
      if (!i==nrow(meta_night)){
        ...
        meta_pause$length = as.numeric(seconds(as.POSIXct(meta_pause$end_time,tz = timezone)) - seconds(as.POSIXct(meta_pause$start_time, tz = timezone))) # length of pause
    ...
    plot = ggplot(toplot,aes(x = as.POSIXct(start_time,tz = timezone),y = yaxis,colour = status)) + 
      ...
      scale_x_datetime(date_breaks = '2 hours',minor_breaks = '30 mins',
                       limits = c(min(as.POSIXct(meta_night$start_time,tz = timezone)) - minutes(60),
                                  max(as.POSIXct(meta_night$start_time,tz = timezone)) + minutes(60)),
                       date_labels = "%H %M") +
      ylim(0,2) +
      ...
      annotate('rect', xmin=as.POSIXct(sunset$sunset,tz = timezone),
               xmax=as.POSIXct(sunrise$sunrise,tz = timezone), ymin=1.2, ymax=Inf, alpha=.2, fill='darkorchid4') +
      
      
      annotate('rect', xmin=min(as.POSIXct(meta_night$start_time,tz = timezone)) - minutes(60),
               xmax=as.POSIXct(sunset$sunset,tz = timezone), ymin=1.2, ymax=Inf, alpha=.2, fill='darkorange3') +
      
      annotate('rect', xmin=as.POSIXct(sunrise$sunrise,tz = timezone),
               xmax=max(as.POSIXct(meta_night$start_time,tz = timezone)) + minutes(60), ymin=1.2, ymax=Inf, alpha=.2, fill='darkorange3') +
      
      geom_linerange(aes(y = yaxis, xmin = as.POSIXct(start_time,tz = timezone),
                         xmax = as.POSIXct(end_time,tz = timezone)),linewidth = 3) +
      
      geom_text(aes(x=as.POSIXct(start_time,tz = timezone)+length/2,label = ifelse(length>60,
                                                           round(length/60,0.1),
                                                           paste0(floor(length)," Sec")), 
                                                          vjust = ifelse(status == "recording",-1.5,+2))) + 
      geom_linerange(aes(x=as.POSIXct(start_time,tz = timezone), ymin = 0, ymax = 0.6, linetype = status), linewidth = 1.2) +
...
write.csv(sessions,file = "S:/ProjectScratch/398-173.07/PMRA_WESOke/PMRA_SAR/2023_WLRS_Contract/processing/recording_tracking/2022_Nawhitti_recording_data_recs_pauses_final_True_Tz.csv",row.names = F)
